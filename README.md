...
We compare the multinomial and complement naive Bayes classifiers to support vector machines, and in particular to SVC. We consider the weighted [F1 score](https://en.wikipedia.org/wiki/F1_score) as the main metric to measure performance, to account for class distribution imbalance. While the naive Bayes classifiers are significantly faster in training and prediction, the support vector machine performs slightly better in prediction. The speed difference becomes more significant as the dataset grows larger in size, therefore using a naive Bayes classifier may be a better choice in production environments. We do not have any time constraint, therefore we choose support vector machines for our classification problem.
